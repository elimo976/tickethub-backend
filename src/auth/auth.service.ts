...
export class AuthService {
    constructor(@InjectModel(User.name) private userModel: Model<UserDocument>, // @InjectModel di Mongoose, fornisce direttamente accesso alla collezione User attraverso userModel
                private jwtService: JwtService
            ) {}

    // auth/auth.service.ts
async register(createUserDto: CreateUserDto): Promise<{ access_token?: string; message: string }> {
  ...
  async login(email: string, password: string): Promise<{ accessToken: string }> {
    try {
        const user = await this.userModel.findOne({ email: email }).select('+password'); // per validare la password devo volontariamente includerla
